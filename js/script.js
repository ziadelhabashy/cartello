...
if (currentUser) {
  ...
  if (addressField && !addressField.value && currentUser.id) {
    fetch(`http://localhost:5000/api/addresses/${currentUser.id}`)
      .then(res => res.json())
      .then(addresses => {
...
async function removeAddress(addressId) {
  ...
  try {
    await fetch('http://localhost:5000/api/remove-address', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function deleteProduct(productId) {
  ...
  try {
    await fetch(`http://localhost:5000/api/admin/products/${productId}`, { method: 'DELETE' });
    loadAdminProducts();
